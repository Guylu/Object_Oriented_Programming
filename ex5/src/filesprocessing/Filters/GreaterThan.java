...
/**
 * class to represent filter that filters files which size is larger than the given number
 */
public class GreaterThan extends Filter {
     ...
     */
    public GreaterThan(String command, boolean isNot) throws Type1 {
        this.isNot = isNot;
        try {
        ...
        if (size < 0)
            throw new NegativeFilterValue("invalid values for greater_than - negative values are invalid");
